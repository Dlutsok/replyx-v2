        ...
        if relevant_chunks:
            ...
            if context_parts:
                ...
                prompt_messages.insert(1, {
                    "role": "system", 
                    "content": f"КРИТИЧЕСКИ ВАЖНО: Используй ИСКЛЮЧИТЕЛЬНО следующую информацию из базы знаний компании. КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО использовать общие знания или информацию из тренировочных данных. Если ответ не найден в предоставленной информации - скажи об этом прямо.\n\nОБЯЗАТЕЛЬНО: Обращайтесь к клиентам на «ВЫ» (говорите «Проверьте», «Обратитесь», «Попробуйте»), а НЕ на «ты» («проверь», «обратись», «попробуй»). Используйте вежливый деловой стиль.\n\n{docs_text}"
                })
