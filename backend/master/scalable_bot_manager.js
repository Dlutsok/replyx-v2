...
class ScalableBotManager {
    ...
    async startBotWorker(botId, botConfig) {
        try {
            ...
            const workerPath = path.join(__dirname, '../worker/bot_worker.js');
    ...
    startAPI() {
        ...
        app.listen(3001, () => {
            console.log('🌐 Scalable Bot Manager API запущен на порту 3001');
        });
    ...
    async cleanupZombieProcesses() {
        try {
            ...
            const { exec } = require('child_process');
            const commands = [
                `ps aux | grep "worker/bot_worker.js" | grep -v grep`,
