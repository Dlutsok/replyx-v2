...
export default function AssistantDetails({
  ...
  const renderDocuments = () => (
    <div className="space-y-4">
      ...
      {/* Website ingest controls */}
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_auto] gap-4 items-end">
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">URL страницы сайта</label>
          <input
            type="url"
        ...
        <motion.button
          ...
          whileTap={{ scale: 0.98 }}
          className="inline-flex items-center gap-2 px-4 py-2 bg-white border border-gray-300 text-gray-700 rounded-xl hover:bg-gray-50 text-sm font-medium transition-all whitespace-nowrap"
          onClick={async (e) => {
            const btn = e.currentTarget;
            try {
              const input = document.getElementById('assistant-website-url');
              const url = (input?.value || '').trim();
              if (!url) { showError('Укажите URL страницы', { title: 'Ошибка' }); return; }
              try { const u = new URL(url); if (!['http:', 'https:'].includes(u.protocol)) throw new Error(); } catch { showError('Некорректный URL', { title: 'Ошибка URL' }); return; }

              // Single-page индексация
              btn.disabled = true;
              btn.textContent = 'Индексируем страницу...';

              const resp = await fetch(`/api/assistants/${assistant.id}/ingest-website`, {
                method: 'POST',
                headers: {
              ...
              if (resp.ok) {
                ...
                // Показать информацию о созданном документе
                btn.textContent = 'Готово ✓';
                btn.className = 'inline-flex items-center gap-2 px-4 py-2 bg-green-600 text-white rounded-xl text-sm font-medium';
                input.value = '';

                // Показать уведомление с деталями
                ...
                // Обновить список документов если есть callback
                if (onRefreshData) {
                  onRefreshData();
                }
              } else {
              ...
              setTimeout(() => {
                btn.disabled = false;
                btn.textContent = 'Индексировать страницу';
                btn.className = 'inline-flex items-center gap-2 px-4 py-2 bg-white border border-gray-300 text-gray-700 rounded-xl hover:bg-gray-50 text-sm font-medium transition-all whitespace-nowrap';
              }, 1500);
            }
          }}
        >
